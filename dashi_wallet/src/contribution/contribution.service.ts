    ...
    try {
      ...
      if (group.status !== 'active') {
        throw new BadRequestException(
          `Cannot contribute to a group with status: ${group.status}`,
        );
      }
      ...
      if (now < group.startDate || now > group.endDate) {
        throw new BadRequestException(
          'This group is currently not in an active contribution cycle',
        );
      }
      ...
      if (amount < group.contributionAmount) {
        throw new BadRequestException(
          `Minimum contribution is ₦${group.contributionAmount}`,
        );
      }
